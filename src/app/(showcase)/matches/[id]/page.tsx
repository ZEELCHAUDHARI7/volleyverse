...
export default function PublicMatchReport() {
  ...
  if (!match) {
    // Either doesn't exist or isn't published — same answer publicly (FR4).
    return (
  ...
  const roster = db.players.filter((p) => match.roster.includes(p.id));
  ...
  const name = (pid?: string) =>
    pid ? (db.players.find((p) => p.id === pid)?.name ?? "N/A") : "N/A";
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-20 md:px-8">
          <Reveal>
            ...
            <h1 className="stat-display text-5xl font-extrabold uppercase leading-[0.95] sm:text-7xl">
              ...
              <br />
              <span className="text-gradient">{match.opponent}</span>
            </h1>
